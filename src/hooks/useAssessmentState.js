    ...
    const SUBMITTED_KEY_PREFIX = "situational_assessment_submitted_";
    ...
    function getInitialState(candidateToken, TOTAL, resetRequested) {
      const submittedKey = SUBMITTED_KEY_PREFIX + (candidateToken || "default");
      if (resetRequested) {
        try {
        ...
        return { answers: {}, idx: 0, monitoringAgreed: false, aiConsent: null, isAlreadySubmitted: false };
      }

      // Check if assessment was already submitted locally
      try {
        const isSubmitted = localStorage.getItem(submittedKey);
        if (isSubmitted === "true") {
          return { answers: {}, idx: TOTAL, monitoringAgreed: true, aiConsent: true, isAlreadySubmitted: true };
        }
      } catch (err) {
        console.warn("Failed to read submission lock key:", err);
      }
    ...
    export function useAssessmentState() {
      ...
      const [alreadyCompleted, setAlreadyCompleted] = useState(initial.isAlreadySubmitted);
      ...
      // Submit assessment and lock token permanently upon completion
      const submitted = idx >= TOTAL || alreadyCompleted;
      useEffect(() => {
        if (submitted) {
          ...
          try {
            ...
            console.warn("Failed to set completion key:", err);
          }
