...
import { Video, Circle, Square, RotateCcw, Check, Film, ArrowRight } from "lucide-react";
...
export function VideoStep({ item, answer, setAnswer, stream, streamStatus, onAutoNext }) {
  ...
  function startRecording() {
    if (!stream) {
      setRecordError("Camera and microphone access is required before recording.");
      return;
    }
    if (typeof MediaRecorder === "undefined") {
      setRecordError("This browser does not support video recording. Please use the latest Chrome or Edge browser.");
      return;
    }
    try {
      setRecordError("");
      ...
      mr.onstop = async () => {
        ...
        // Instant Blob ObjectURL for 100% reliable local video preview
        const objUrl = URL.createObjectURL(blob);
        setPreviewUrl(objUrl);
        setRecStatus("done");
        clearInterval(timerRef.current);

        // Save physical video file into public/temp_videos in project directory
        try {
          setSavingFile(true);
      ...
      setRecordError("Your browser could not start the recorder. Please verify camera/microphone access.");
    }
  }
  ...
  return (
    <div className="animate-fade-in" style={{ maxWidth: 680, margin: "0 auto" }}>
      ...
      {/* Compact Video Viewport (Reduced by 40% in height & width for single page view) */}
      <div
        style={{
          width: "100%",
          maxWidth: 480,
      ...
      >
        ...
        {!stream && streamStatus !== "denied" && !previewUrl && (
          <div style={{ color: "#B8C1D1", textAlign: "center", fontSize: 13 }}>
            <Video size={28} style={{ marginBottom: 6 }} />
            <div>Camera preview will appear here</div>
          </div>
        )}

        {!stream && streamStatus === "denied" && !previewUrl && (
          <div style={{ color: "#F3A5A5", textAlign: "center", fontSize: 12.5, padding: 16 }}>
            Camera and microphone access was not granted. Go back to the permission step to allow access.
          </div>
        )}
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 14, alignItems: "center", justifyContent: "center", flexWrap: "wrap" }}>
        ...
        {recStatus === "submitted" && (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <span style={{ display: "flex", alignItems: "center", gap: 6, color: "var(--color-success)", fontSize: 13.5, fontWeight: 700, background: "var(--color-success-bg)", padding: "8px 14px", borderRadius: 8 }}>
      ...
      {recordError && (
        <div style={{ marginTop: 10, color: "var(--color-danger)", background: "var(--color-danger-bg)", border: "1px solid #F5C2C0", borderRadius: 8, padding: "8px 12px", fontSize: 12.5, textAlign: "center" }}>
          {recordError}
        </div>
      )}
    </div>
